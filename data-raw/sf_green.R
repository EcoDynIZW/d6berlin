...
sf_green <-
  suppressMessages(
    ...
    sf::read_sf(
      dsn = paste0(shp_path, "/gis_osm_landuse_a_free_1.shp"),
      layer = "gis_osm_landuse_a_free_1"
    ) |>
      ## Berlin Natural Areas (WGS 84)
      rbind(sf::read_sf(
        dsn = paste0(shp_path, "/gis_osm_natural_a_free_1.shp"),
        layer = "gis_osm_natural_a_free_1"
      )) |>
      dplyr::filter(fclass %in% c(
        "forest", "grass", "meadow", "nature_reserve",
