...
globe <- function(center = c(13.4050, 52.5200), col_earth = "#a5bf8b", col_water = "#96b6d8",
  ...
  if (identical(center, c(13.4050, 52.5200))) {
    lat <- 32
  }
  ...
  visible <- suppressMessages(suppressWarnings(
    sf::st_intersection(
      sf::st_make_valid(d6berlin::sf_world),
  ...
  globe <- globe +
    ...
    ggplot2::geom_sf(
      data = sf::st_collection_extract(visible),
      fill = col_earth, color = "transparent"
    ) +
    ggplot2::geom_sf(data = sf_inner, color = col_earth, lwd = .3) +
    ggplot2::geom_sf(
      data = sf_berlin_loc, color = col_pin, size = size_pin,
